...
def get_active_nodes_colosseumcli() -> dict:
    ...
    print("PPR from get_active_nodes_colosseumcli function")
    print(active_nodes)
    print("##########################")
    return active_nodes
...
def get_active_nodes_nmap() -> dict:
    ...
    print("PPR from get_active_nodes_nmap function")
    print(active_nodes)
    for key, val in active_nodes.items():
        nodes_parameters[key] = {"node_id": key, "srn_id": val}
    current_col0_base_ip = col0_base_ip
    print("##########################")
...
def get_nodes_ip(active_nodes: list) -> tuple:
    ...
    print("PPR from get_nodes_ip function")
    print("my_ip", my_ip)
    print("my_node_id", my_node_id)
    print("nodes_ip",nodes_ip)
    print("##########################")
    # nodes_parameters["my_node_id"] = my_node_id
    nodes_parameters[my_node_id]["this_is_my_node"] = True
    nodes_parameters[my_node_id]["col_ip"] = my_ip

    for node_id, col_ip in nodes_ip.items():
        # col_ip = "172.30.101." + str(100 + node_id)
        nodes_parameters[node_id]["col_ip"] = col_ip
        nodes_parameters[node_id]["this_is_my_node"] = False
        # print("node_id", node_id, "------", "my_node_id", my_node_id)
...
def get_my_users_ip(my_node_id: int, bs_ue_num: int, nodes_ip: dict) -> dict:
    ...
    print("PPR from get_my_users_ip function")
    print("my_users_ip",my_users_ip)
    print("##########################")
    return my_users_ip
...
def is_node_bs(bs_ue_num: int, use_colosseumcli: bool) -> tuple:
    ...
    print("PPR from is_node_bs function")
    print("is_bs",is_bs)
    print("my_ip", my_ip)
    print("my_node_id", my_node_id)
    print("nodes_ip", nodes_ip)
    print("bs_ue_num", bs_ue_num)
    print("##########################")
    # nodes_parameters["bs_ue_num"] = bs_ue_num
    if is_bs:
        for node_id, node_dict in nodes_parameters.items():
            print(node_id, "-----", node_dict)
            if node_id == my_node_id:
                nodes_parameters[node_id]["node_type"] = "bs"
                nodes_parameters[node_id]["lte_ip"] = "172.16.0.1"
            else:
                nodes_parameters[node_id]["node_type"] = "ue"
    else:
        for node_id, node_dict in nodes_parameters.items():
            # everyone is "ue" until we figure out who is the "bs" later.
            nodes_parameters[node_id]["node_type"] = "ue"
...
def start_iperf_client(tmux_session_name: str, server_ip: str, client_ip: str) -> None:
    ...
    iperf_cmd = 'iperf3 -c ' + server_ip + ' -p ' + str(port) + ' -u -b 10M -t 600 -R'
...
def start_iperf_server(client_ip) -> None:
    ...
    for c_ip in client_ip:
        ...
        for node_id, node_dict in nodes_parameters.items():
            if c_ip == node_dict["lte_ip"]:
                node_dict["iperf_port"] = port
                break
...
def run_scope(bs_ue_num: int, iperf: bool, use_colosseumcli: bool,
    ...
    os.system('tmux kill-session -t speedtest')
    os.system("rm -rf ue_datasets")
    os.system("rm -rf nodes_config_files")
    os.system("mkdir ue_datasets")
    os.system("mkdir nodes_config_files")

    is_bs, my_ip, my_node_id, nodes_ip, bs_ue_num = is_node_bs(bs_ue_num, use_colosseumcli)

    print("My node ID is:", my_node_id)
    print("##########################")
    ...
    if is_bs:
        ...
        # Adding the lte_ip addresses based on the col_ip addresses.
        for key, node_dict in nodes_parameters.items():
            for col_ip, srs_ip in srs_col_ip_mapping.items():
                if node_dict["col_ip"] == col_ip:
                    # print(val)
                    # print(ival)
                    node_dict["lte_ip"] = srs_ip
                    break  # Breaks the inner loops only.

        # # Adding the IMSI for each user.
        # for key, val in nodes_parameters.items():
        #     for ikey, ival in srs_imsi_id_mapping.items():
        #         if key == ival:
        #             val["ue_imsi"] = ikey
        #             break

        for imsi, node_id in srs_imsi_id_mapping.items():
            nodes_parameters[node_id]["ue_imsi"] = imsi
        ...
        # The server (BS/sender) speedtest script.
        ...
        # get mapping of srsLTE UE addresses and IPs
        srs_col_ip_mapping, srs_imsi_id_mapping = get_srsue_ip_mapping(bs_id, nodes_ip, srslte_config_dir)
        ...
        # Adding the IMSIs for the UEs.
        for imsi, node_id in srs_imsi_id_mapping.items():
            nodes_parameters[node_id]["ue_imsi"] = imsi

        # compute my srsLTE IP and extract it from the returned dictionary
        my_srslte_ip, my_imsi_mapping = get_srsue_ip_mapping(bs_id, {my_node_id: my_ip}, srslte_config_dir)
        my_srslte_ip = my_srslte_ip[my_ip]
        logging.info('My srsLTE IP: ' + my_srslte_ip)

        # Adding my own lte_ip
        nodes_parameters[my_node_id]["lte_ip"] = my_srslte_ip

        # Adding my own IMSI
        my_imsi = my_imsi_mapping.popitem()[0]
        nodes_parameters[my_node_id]["ue_imsi"] = my_imsi

        # Update BS node type to "bs"
        nodes_parameters[bs_id]["node_type"] = "bs"
        ...
        # The client (UE/receiver) script.
        # time.sleep(10)


# Separating the parameters of nodes into separate files.
def transfer_node_files(nodes_parameters):
    # Directory created in run_scope().
    ...
    for node_id, node_dict in nodes_parameters.items():
        if node_dict["node_type"] == "ue":
            ...
            node_file_path = config_files_path + str(node_id) + "_config_file.txt"
            config_file = open(node_file_path, "w")
            json.dump(config_dict, config_file, indent=4)
            config_file.close()

            # Creating traffic injection files.
            # traffic_dict = {}
            # traffic_dict["packet_size"] = 0
            # traffic_dict["desired_bandwidth"] = 0
            # traffic_dict["my_slice_resources"] = 0
            # traffic_dict["requested_slice_resources"] = 0
            node_file_path = config_files_path + str(node_id) + "_traffic_file.txt"
            config_file = open(node_file_path, "w")
            json.dump(config_dict, config_file, indent=4)
            config_file.close()
            ...
            model_dict = {}
            model_dict["action"] = 0
            model_dict["action_values"] = []
            node_file_path = config_files_path + str(node_id) + "_model_file.txt"
            config_file = open(node_file_path, "w")
...
if __name__ == '__main__':
    ...
    json_config_file = open("radio_api/radio_interactive.conf")
    network_parameters = json.load(json_config_file)
    json_config_file.close()

    slice_allocation = network_parameters["slice-allocation"]
    slice_users = network_parameters["slice-users"]
    slice_allocation = ast.literal_eval(slice_allocation)
    slice_users = ast.literal_eval(slice_users)

    # Fixing the error of IPs and replacing tr0 with the true col_ip.
    global current_col0_base_ip
    for node_id, node_dict in nodes_parameters.items():
        # print("node_id", node_id)
        # print("node_dict", node_dict)
        node_dict["col_ip"] = current_col0_base_ip + str(100 + node_dict["srn_id"])
        # Adding additional packet injection parameters.
        if node_dict["node_type"] == "ue":
            node_dict["packet_size"] = 50000
            node_dict["desired_bandwidth"] = 500000
            node_dict["sum_requested_prbs"] = 0
            node_dict["sum_granted_prbs"] = 0
            node_dict["requested_slice_resources"] = 0
            # node_dict["action"] = 0
            # node_dict["action_values"] = 0
            # node_dict["actual_bandwidth"] = 0

            # Figuring out which slice this UE belongs to.
            node_dict["my_slice_id"] = -1
            for slice_id, slice_user_list in slice_users.items():
                # print("slice_id", slice_id)
                # print("slice_user_list", slice_user_list)
                if int(node_id) in slice_user_list:
                    node_dict["my_slice_id"] = slice_id

                    # Figuring out how many PRBs my slice have.
                    my_slice_id = node_dict["my_slice_id"]
                    my_slice_resources = slice_allocation[my_slice_id][-1] - slice_allocation[my_slice_id][0] + 1
                    node_dict["my_slice_resources"] = my_slice_resources
                    break

            if node_dict["my_slice_id"] == -1:
                node_dict["node_type"] = "unknown"


    nodes_parameters_file = open("nodes_parameters_file.txt", "w")
    json.dump(nodes_parameters, nodes_parameters_file, indent=4)
    nodes_parameters_file.close()

    transfer_node_files(nodes_parameters)
    ...
    os.system('cat nodes_parameters_file.txt')

    # tmux_speedtest = 'speedtest'
    #
    # if node_type == "ue":
    #     receiver_speedtest_cmd = 'python3 /root/radio_api/receiver_client_speed_test.py'
    #     run_tmux_command(receiver_speedtest_cmd, tmux_speedtest)
    #     print("running UE speedtest script")
    # if node_type == "bs":
    #     sender_speedtest_cmd = 'python3 /root/radio_api/sender_server_speed_test.py'
    #     run_tmux_command(sender_speedtest_cmd, tmux_speedtest)
    #     print("running BS speedtest script")
