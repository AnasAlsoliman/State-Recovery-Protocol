...
def get_srsue_ip_mapping(bs_id: int, nodes_ip: dict, srslte_config_dir: str) -> tuple:
    ...
    print("PPR from get_srsue_ip_mapping function")
    print("srs_col_ip_mapping",srs_col_ip_mapping)
    print("srs_imsi_id_mapping", srs_imsi_id_mapping)
    print("##########################")
...
def ppr_read_metrics(lines_num: int=None, ue_imsi: str=None) -> {}:
    ...
    if ue_imsi:  # currently not used
        ...
        for el in dir_list:
            ...
            if filename in el:
                ...
                #  Ignore RNTIs and process IMSI metrics only
                if len(curr_imsi) >= min_imsi_len:
                    metrics_file = path + curr_imsi + filename
                    csv_file = open(metrics_file)
